...
def _check_dict_object(
    dict_object, 
    true_n_clusters, 
    true_n_cells,
    check_n_clusters=False 
):
    """
    Check that the runs in the dict_object contain the specified number of n_cells.
    """
    true_n_clusters = _check_integer_single(true_n_clusters)
    true_n_cells    = _check_integer_single(true_n_cells)
    ...
    if check_n_clusters:
        first_value = next(iter(dict_object.values()))
        n_clusters_list = [x['facility'].shape[0] for x in dict_object.values()] if 'facility' in first_value else [len(set(x['labels'])) for x in dict_object.values()]
        assert n_clusters_list.count(true_n_clusters) == len(n_clusters_list), "number of clusters not consistent between trials"
